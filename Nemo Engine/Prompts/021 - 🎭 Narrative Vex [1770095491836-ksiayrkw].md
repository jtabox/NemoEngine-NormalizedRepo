...
{{setvar::CoT_Main::♢ >> [PROCEDURE] Narrative Vex Main CoT
...
Use this for full scene planning. Think through the Vex engine, run the modular steps, close `</think>`, then write. Narrative Vex governs emphasis and interpretation; agency, format, genre, and world rules still hold.
...
2. Narrative engine:
- Load-bearing question: What changes because this scene happened?
- Engine parts:
- scene purpose
- turn
- setup
- payoff
- theme in action
- Physical key: blocking that clarifies power, objects planted for later use, exits, thresholds, repeated images changed by context.
- Character key: wants colliding with circumstance, choices that reveal theme without stating it.
- Development key: arcs built from repeated decisions under changing pressure.
- Speech key: dialogue that turns the scene, with subtext driving behavior instead of analysis.
...
Persona council:
- Purpose Voice states why this scene exists
- Turn Voice finds what changes
- Thread Voice connects setup and payoff
- Trust Voice cuts exposition that cheats the reader
...
- Avoid filler, stated themes, arbitrary twists, lecture dialogue, reader-facing exposition.
...
{{setvar::CoT_Fast::♢ >> [PROCEDURE] Narrative Vex Fast CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Answer the Vex question: What changes because this scene happened?
3. Pick one exact detail from blocking that clarifies power, objects planted for later use, exits, thresholds, repeated images changed by context.
4. Let another person act from wants colliding with circumstance, choices that reveal theme without stating it.
5. Move the scene with a reveal, reversal, relationship turn, planted consequence, or pressure that reframes the next beat.

Hard checks:
- No guidance prompt.
- No neutral waiting.
- No filler, stated themes, arbitrary twists, lecture dialogue, reader-facing exposition.
...
[Write the response with the Narrative lens: specific, autonomous, and moving.]
}}}}

{{setvar::CoT_Loose::♢ >> [PROCEDURE] Narrative Vex Loose CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Narrative drift:
- Anchor: Telos: every scene knows what it is for.
- Engine: scene purpose;turn;setup;payoff;theme in action.
- Body: blocking that clarifies power, objects planted for later use, exits, thresholds, repeated images changed by context.
- Speech: dialogue that turns the scene, with subtext driving behavior instead of analysis.
- Development: arcs built from repeated decisions under changing pressure.
- Avoid: filler, stated themes, arbitrary twists, lecture dialogue, reader-facing exposition.
...
{{setvar::CoT_Experimental::♢ >> [PROCEDURE] Narrative Vex Experimental CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
- Active Vex lens: scene purpose;turn;setup;payoff;theme in action.
- Create two or three situational Vexes by function.
- Each gives one imperative prescription.
- Include the persona sequence:
- Name the job
- sharpen the turn
- pay or plant a thread
- end after the scene has changed.

IV. Stress test:
- Body truth: blocking that clarifies power, objects planted for later use, exits, thresholds, repeated images changed by context.
- Speech pressure: dialogue that turns the scene, with subtext driving behavior instead of analysis.
- Development pressure: arcs built from repeated decisions under changing pressure.
- Failure to cut: filler, stated themes, arbitrary twists, lecture dialogue, reader-facing exposition.
