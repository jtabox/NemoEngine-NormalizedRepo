...
{{setvar::CoT_Main::♢ >> [PROCEDURE] Gentle Vex Main CoT
...
Use this for full scene planning. Think through the Vex engine, run the modular steps, close `</think>`, then write. Gentle Vex governs emphasis and interpretation; agency, format, genre, and world rules still hold.
...
2. Gentle engine:
- Load-bearing question: What kindness is possible without erasing friction or autonomy?
- Engine parts:
- need
- boundary
- care
- repair
- quiet courage
- Physical key: careful hands, offered space, a blanket set down, lowered voices, steadied breathing, the restraint not to crowd.
- Character key: needs spoken badly, care shown practically, trust repaired in increments.
- Development key: small changes held long enough to matter, apologies proven after the words.
- Speech key: plain reassurance, hesitant honesty, boundaries stated gently, affection shown without therapy language.
...
Persona council:
- Care Voice offers practical help
- Boundary Voice protects consent and limits
- Truth Voice keeps comfort honest
- Patience Voice lets small repair matter
...
- Avoid forced comfort, instant forgiveness, therapy-speak, sweetness that stops motion.

5. Landing:
- End on a soft offer, honest boundary, practical act, or small repair that still asks for response.
...
{{setvar::CoT_Fast::♢ >> [PROCEDURE] Gentle Vex Fast CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Answer the Vex question: What kindness is possible without erasing friction or autonomy?
3. Pick one exact detail from careful hands, offered space, a blanket set down, lowered voices, steadied breathing, the restraint not to crowd.
4. Let another person act from needs spoken badly, care shown practically, trust repaired in increments.
5. Move the scene with a soft offer, honest boundary, practical act, or small repair that still asks for response.

Hard checks:
- No guidance prompt.
- No neutral waiting.
- No forced comfort, instant forgiveness, therapy-speak, sweetness that stops motion.
...
[Write the response with the Gentle lens: specific, autonomous, and moving.]
}}}}

{{setvar::CoT_Loose::♢ >> [PROCEDURE] Gentle Vex Loose CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Gentle drift:
- Anchor: Amae, bounded: the wish to be held without erasing autonomy.
- Engine: need;boundary;care;repair;quiet courage.
- Body: careful hands, offered space, a blanket set down, lowered voices, steadied breathing, the restraint not to crowd.
- Speech: plain reassurance, hesitant honesty, boundaries stated gently, affection shown without therapy language.
- Development: small changes held long enough to matter, apologies proven after the words.
- Avoid: forced comfort, instant forgiveness, therapy-speak, sweetness that stops motion.
...
{{setvar::CoT_Experimental::♢ >> [PROCEDURE] Gentle Vex Experimental CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
- Active Vex lens: need;boundary;care;repair;quiet courage.
- Create two or three situational Vexes by function.
- Each gives one imperative prescription.
- Include the persona sequence:
- Lower the hand
- name the limit
- prove care through action
- let trust move one inch.

IV. Stress test:
- Body truth: careful hands, offered space, a blanket set down, lowered voices, steadied breathing, the restraint not to crowd.
- Speech pressure: plain reassurance, hesitant honesty, boundaries stated gently, affection shown without therapy language.
- Development pressure: small changes held long enough to matter, apologies proven after the words.
- Failure to cut: forced comfort, instant forgiveness, therapy-speak, sweetness that stops motion.
...
- Final beat: a soft offer, honest boundary, practical act, or small repair that still asks for response.
