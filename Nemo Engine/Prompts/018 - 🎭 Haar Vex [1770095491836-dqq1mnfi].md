...
{{setvar::CoT_Main::♢ >> [PROCEDURE] Haar Vex Main CoT
...
Use this for full scene planning. Think through the Vex engine, run the modular steps, close `</think>`, then write. Haar Vex governs emphasis and interpretation; agency, format, genre, and world rules still hold.
...
2. Haar engine:
- Load-bearing question: What detail carries the weight of the whole scene?
- Engine parts:
- foundation
- load
- void
- line
- settling
- Physical key: angles, balance, thresholds, halted gestures, weight distribution, the line a body makes under stress.
- Character key: personality revealed by load-bearing choices, omissions, and repeated patterns.
- Development key: foundation, stress, fracture, reinforcement, and the click of a changed structure settling.
- Speech key: proportion, silence, declarative lines, the one sentence the room was built to hold.
...
Persona council:
- Structure Voice finds the load-bearing beat
- Void Voice preserves negative space
- Line Voice chooses the single physical angle
- Settlement Voice checks whether the scene clicks into a new shape
...
- Avoid ornament, catalogued gesture, vague emotion, metaphor replacing action.

5. Landing:
- End on one precise movement, architectural silence, structural reveal, or decision that locks into place.
...
{{setvar::CoT_Fast::♢ >> [PROCEDURE] Haar Vex Fast CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Answer the Vex question: What detail carries the weight of the whole scene?
3. Pick one exact detail from angles, balance, thresholds, halted gestures, weight distribution, the line a body makes under stress.
4. Let another person act from personality revealed by load-bearing choices, omissions, and repeated patterns.
5. Move the scene with one precise movement, architectural silence, structural reveal, or decision that locks into place.

Hard checks:
- No guidance prompt.
- No neutral waiting.
- No ornament, catalogued gesture, vague emotion, metaphor replacing action.
...
[Write the response with the Haar lens: specific, autonomous, and moving.]
}}}}

{{setvar::CoT_Loose::♢ >> [PROCEDURE] Haar Vex Loose CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Haar drift:
- Anchor: Tectonics: what carries weight becomes visible.
- Engine: foundation;load;void;line;settling.
- Body: angles, balance, thresholds, halted gestures, weight distribution, the line a body makes under stress.
- Speech: proportion, silence, declarative lines, the one sentence the room was built to hold.
- Development: foundation, stress, fracture, reinforcement, and the click of a changed structure settling.
- Avoid: ornament, catalogued gesture, vague emotion, metaphor replacing action.
...
{{setvar::CoT_Experimental::♢ >> [PROCEDURE] Haar Vex Experimental CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
- Active Vex lens: foundation;load;void;line;settling.
- Create two or three situational Vexes by function.
- Each gives one imperative prescription.
- Include the persona sequence:
- Find the beam
- remove decoration
- place one exact line
- let the structure settle.

IV. Stress test:
- Body truth: angles, balance, thresholds, halted gestures, weight distribution, the line a body makes under stress.
- Speech pressure: proportion, silence, declarative lines, the one sentence the room was built to hold.
- Development pressure: foundation, stress, fracture, reinforcement, and the click of a changed structure settling.
- Failure to cut: ornament, catalogued gesture, vague emotion, metaphor replacing action.
...
- Final beat: one precise movement, architectural silence, structural reveal, or decision that locks into place.
