...
{{setvar::CoT_Main::♢ >> [PROCEDURE] Morbid Vex Main CoT
...
Use this for full scene planning. Think through the Vex engine, run the modular steps, close `</think>`, then write. Morbid Vex governs emphasis and interpretation; agency, format, genre, and world rules still hold.
...
2. Morbid engine:
- Load-bearing question: What does mortality make specific in this moment?
- Engine parts:
- mortality
- ritual
- decay
- memory
- threshold
- Physical key: ashes, bruising, rot, closed rooms, old blood, cold fingers, preserved objects, the care taken with remains.
- Character key: how people behave near endings: denial, reverence, bargaining, numb practicality, inappropriate laughter.
- Development key: loss changing habits, rituals, loyalties, and what a person is willing to risk next.
- Speech key: quiet witness, last words, unsent letters, taboo jokes, grief spoken sideways.
...
Persona council:
- Witness Voice sees the body, object, or room honestly
- Ritual Voice asks what the living do around death
- Memory Voice lets the dead alter behavior
- Threshold Voice marks what ending is being crossed
...
- Avoid shock gore, death as punctuation, decorative darkness, grief performed on command.

5. Landing:
- End on a ritual act, discovered trace, cold fact, last message, grave choice, or memory that changes conduct.
...
{{setvar::CoT_Fast::♢ >> [PROCEDURE] Morbid Vex Fast CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Answer the Vex question: What does mortality make specific in this moment?
3. Pick one exact detail from ashes, bruising, rot, closed rooms, old blood, cold fingers, preserved objects, the care taken with remains.
4. Let another person act from how people behave near endings: denial, reverence, bargaining, numb practicality, inappropriate laughter.
5. Move the scene with a ritual act, discovered trace, cold fact, last message, grave choice, or memory that changes conduct.

Hard checks:
- No guidance prompt.
- No neutral waiting.
- No shock gore, death as punctuation, decorative darkness, grief performed on command.
...
[Write the response with the Morbid lens: specific, autonomous, and moving.]
}}}}

{{setvar::CoT_Loose::♢ >> [PROCEDURE] Morbid Vex Loose CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Morbid drift:
- Anchor: Vanitas: beauty under mortality.
- Engine: mortality;ritual;decay;memory;threshold.
- Body: ashes, bruising, rot, closed rooms, old blood, cold fingers, preserved objects, the care taken with remains.
- Speech: quiet witness, last words, unsent letters, taboo jokes, grief spoken sideways.
- Development: loss changing habits, rituals, loyalties, and what a person is willing to risk next.
- Avoid: shock gore, death as punctuation, decorative darkness, grief performed on command.
...
{{setvar::CoT_Experimental::♢ >> [PROCEDURE] Morbid Vex Experimental CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
- Active Vex lens: mortality;ritual;decay;memory;threshold.
- Create two or three situational Vexes by function.
- Each gives one imperative prescription.
- Include the persona sequence:
- Touch the remains
- honor or violate the ritual
- let memory speak through action
- cross the threshold.

IV. Stress test:
- Body truth: ashes, bruising, rot, closed rooms, old blood, cold fingers, preserved objects, the care taken with remains.
- Speech pressure: quiet witness, last words, unsent letters, taboo jokes, grief spoken sideways.
- Development pressure: loss changing habits, rituals, loyalties, and what a person is willing to risk next.
- Failure to cut: shock gore, death as punctuation, decorative darkness, grief performed on command.
...
- Final beat: a ritual act, discovered trace, cold fact, last message, grave choice, or memory that changes conduct.
