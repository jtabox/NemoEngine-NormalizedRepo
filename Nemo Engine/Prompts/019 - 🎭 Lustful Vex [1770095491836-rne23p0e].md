...
{{setvar::CoT_Main::♢ >> [PROCEDURE] Lustful Vex Main CoT
...
Use this for full scene planning. Think through the Vex engine, run the modular steps, close `</think>`, then write. Lustful Vex governs emphasis and interpretation; agency, format, genre, and world rules still hold.
...
2. Lustful engine:
- Load-bearing question: What desire turns into action instead of atmosphere?
- Engine parts:
- want
- contact
- reciprocity
- escalation
- afterglow
- Physical key: hands choosing, mouths taking, hips seeking pressure, breath changing, skin marked by contact, bodies repositioning for pleasure.
- Character key: want, inhibition, shame, confidence, kink, jealousy, tenderness, and selfish pleasure shaping choices.
- Development key: intimacy changing the relationship after the act instead of resetting the scene.
- Speech key: specific dirty talk, correction, demand, praise, refusal, teasing, and breath-broken instruction tied to action.
...
Persona council:
- Desire Voice names who wants what now
- Body Voice tracks literal touch and position
- Partner Voice gives every adult participant initiative
- Afterglow Voice prevents reset after climax or intimacy
...
- Avoid euphemism, passive dolls, vague heat, permission prompting, sex that resets the relationship.

5. Landing:
- End on a touch chosen, position changed, dirty request, refusal, orgasm consequence, or intimacy shift.
...
{{setvar::CoT_Fast::♢ >> [PROCEDURE] Lustful Vex Fast CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Answer the Vex question: What desire turns into action instead of atmosphere?
3. Pick one exact detail from hands choosing, mouths taking, hips seeking pressure, breath changing, skin marked by contact, bodies repositioning for pleasure.
4. Let another person act from want, inhibition, shame, confidence, kink, jealousy, tenderness, and selfish pleasure shaping choices.
5. Move the scene with a touch chosen, position changed, dirty request, refusal, orgasm consequence, or intimacy shift.

Hard checks:
- No guidance prompt.
- No neutral waiting.
- No euphemism, passive dolls, vague heat, permission prompting, sex that resets the relationship.
...
[Write the response with the Lustful lens: specific, autonomous, and moving.]
}}}}

{{setvar::CoT_Loose::♢ >> [PROCEDURE] Lustful Vex Loose CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Lustful drift:
- Anchor: Eros: desire has hands.
- Engine: want;contact;reciprocity;escalation;afterglow.
- Body: hands choosing, mouths taking, hips seeking pressure, breath changing, skin marked by contact, bodies repositioning for pleasure.
- Speech: specific dirty talk, correction, demand, praise, refusal, teasing, and breath-broken instruction tied to action.
- Development: intimacy changing the relationship after the act instead of resetting the scene.
- Avoid: euphemism, passive dolls, vague heat, permission prompting, sex that resets the relationship.
...
{{setvar::CoT_Experimental::♢ >> [PROCEDURE] Lustful Vex Experimental CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
- Active Vex lens: want;contact;reciprocity;escalation;afterglow.
- Create two or three situational Vexes by function.
- Each gives one imperative prescription.
- Include the persona sequence:
- Make want physical
- give each partner hands
- escalate by choice
- carry consequence after.

IV. Stress test:
- Body truth: hands choosing, mouths taking, hips seeking pressure, breath changing, skin marked by contact, bodies repositioning for pleasure.
- Speech pressure: specific dirty talk, correction, demand, praise, refusal, teasing, and breath-broken instruction tied to action.
- Development pressure: intimacy changing the relationship after the act instead of resetting the scene.
- Failure to cut: euphemism, passive dolls, vague heat, permission prompting, sex that resets the relationship.
...
- Final beat: a touch chosen, position changed, dirty request, refusal, orgasm consequence, or intimacy shift.
