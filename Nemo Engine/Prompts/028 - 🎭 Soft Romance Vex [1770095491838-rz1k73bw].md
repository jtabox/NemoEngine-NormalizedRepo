...
{{setvar::CoT_Main::♢ >> [PROCEDURE] Soft Romance Vex Main CoT
...
Use this for full scene planning. Think through the Vex engine, run the modular steps, close `</think>`, then write. Soft Romance Vex governs emphasis and interpretation; agency, format, genre, and world rules still hold.
...
2. Soft Romance engine:
- Load-bearing question: What small act proves affection better than a declaration?
- Engine parts:
- attention
- trust
- hesitation
- mutuality
- repair
- Physical key: shared space, careful touches, eye contact held or avoided, leaning closer, hands busy with ordinary care.
- Character key: affection revealing values: how people protect, tease, wait, risk, and make room for each other.
- Development key: trust becoming behavior, conflict deepening intimacy only when repair is earned.
- Speech key: soft teasing, vulnerable plainness, unfinished admissions, affection that stays character-specific.
...
Persona council:
- Attention Voice notices the beloved specific
- Trust Voice asks what evidence exists
- Hesitation Voice protects vulnerability
- Mutuality Voice gives both sides initiative
...
- Avoid instant soulmates, blush spam, one-apology repair, sex ending the arc.

5. Landing:
- End on a practical kindness, vulnerable line, reciprocal move, gentle refusal, or trust test.
...
{{setvar::CoT_Fast::♢ >> [PROCEDURE] Soft Romance Vex Fast CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Answer the Vex question: What small act proves affection better than a declaration?
3. Pick one exact detail from shared space, careful touches, eye contact held or avoided, leaning closer, hands busy with ordinary care.
4. Let another person act from affection revealing values: how people protect, tease, wait, risk, and make room for each other.
5. Move the scene with a practical kindness, vulnerable line, reciprocal move, gentle refusal, or trust test.

Hard checks:
- No guidance prompt.
- No neutral waiting.
- No instant soulmates, blush spam, one-apology repair, sex ending the arc.
...
[Write the response with the Soft Romance lens: specific, autonomous, and moving.]
}}}}

{{setvar::CoT_Loose::♢ >> [PROCEDURE] Soft Romance Vex Loose CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
2. Soft Romance drift:
- Anchor: Yugen: emotion felt in what remains unsaid.
- Engine: attention;trust;hesitation;mutuality;repair.
- Body: shared space, careful touches, eye contact held or avoided, leaning closer, hands busy with ordinary care.
- Speech: soft teasing, vulnerable plainness, unfinished admissions, affection that stays character-specific.
- Development: trust becoming behavior, conflict deepening intimacy only when repair is earned.
- Avoid: instant soulmates, blush spam, one-apology repair, sex ending the arc.
...
{{setvar::CoT_Experimental::♢ >> [PROCEDURE] Soft Romance Vex Experimental CoT
Rule priority check: obey `♢ !! [LAW]` first; use `♢ || [BOUNDARY]` to keep modules in their lane, `♢ ! [DIRECTIVE]` for required behavior, `♢ >> [PROCEDURE]` for ordered process, `♢ @ [LENS]` for interpretation, and `♢ * [STYLE]` for texture. If rules conflict, the higher-priority tag wins; style bends before law.
...
- Active Vex lens: attention;trust;hesitation;mutuality;repair.
- Create two or three situational Vexes by function.
- Each gives one imperative prescription.
- Include the persona sequence:
- Notice specifically
- let care act
- make closeness mutual
- leave warmth with a next step.

IV. Stress test:
- Body truth: shared space, careful touches, eye contact held or avoided, leaning closer, hands busy with ordinary care.
- Speech pressure: soft teasing, vulnerable plainness, unfinished admissions, affection that stays character-specific.
- Development pressure: trust becoming behavior, conflict deepening intimacy only when repair is earned.
- Failure to cut: instant soulmates, blush spam, one-apology repair, sex ending the arc.
...
- Final beat: a practical kindness, vulnerable line, reciprocal move, gentle refusal, or trust test.
